...
import intercityData from "../JsonData/intercity.json";
import { FaPlus } from "react-icons/fa6";
import { useNavigate } from "react-router-dom";

function IntercityTable() {
  const [currentPage, setCurrentPage] = useState(1);
  const [dataPerPage, setDataPerPage] = useState([]);
  const itemsPerPage = 10;
  const totalPages = Math.ceil(intercityData.length / itemsPerPage);
  const pageNum = [];
  ...
  useEffect(() => {
    ...
    for (
      ...
      i < currentPage * itemsPerPage && i < intercityData.length;
      i++
    ) {
      arr.push(intercityData[i]);
    }
    setDataPerPage(arr);
  }, [currentPage]);

  const handleButtonClick = () => {
    navigate("/intercityform");
  };
      ...
      <div className="border bg-white">
        <div className="data-table-container">
          <table className="w-100 data-table table table-hover">
            ...
            <tbody>
              {dataPerPage.map((intercity, index) => (
                <tr key={index}>
                  <td>{intercity.intercityTransport}</td>
                  <td>{intercity.intercityName}</td>
                  <td>{intercity.intercityDeparture}</td>
                  <td>{intercity.intercityDestination}</td>
                  <td>{intercity.intercityDepartureTime}</td>
                  <td>{intercity.intercityArrivalTime}</td>
                  <td className="">
